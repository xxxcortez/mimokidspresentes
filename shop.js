...
const JACK_BUNDLE={qty:3,price:125};
function calcBagTotal(){let subtotal=0,jackQty=0;bag.forEach(x=>{const p=items[x.id];subtotal+=x.quantity*p.price;if(p.promo==='jack')jackQty+=x.quantity});const jackDiscounted=Math.floor(jackQty/JACK_BUNDLE.qty)*JACK_BUNDLE.price+(jackQty%JACK_BUNDLE.qty)*49;const discount=jackQty*49-jackDiscounted;return{subtotal,discount,total:subtotal-discount}}
...
const bagDialog=document.createElement('dialog');bagDialog.className='bag-dialog';bagDialog.setAttribute('aria-labelledby','bag-title');bagDialog.innerHTML='<button class="bag-close close" aria-label="Fechar carrinho">×</button><div class="bag-content"><div class="bag-header-row"><div class="bag-header-text"><p class="eyebrow">SEUS MIMOS</p><h2 id="bag-title">Meu carrinho</h2></div><a class="bag-brand brand-logo" href="#" aria-label="Mimo Kids Presentes, início"><svg class="brand-mark" viewBox="144 524 592 628" aria-hidden="true"><image href="assets/logo-mimo-kids.png" width="900" height="1600"/></svg><span class="brand-wordmark"><strong>Mimo Kids</strong><small>Presentes</small></span></a></div><p class="demo-note">Escolha seus mimos e fale com a gente para confirmar tamanhos e disponibilidade.</p><div id="bag-items"></div><p id="bag-status" role="status"></p><label class="order-observation" for="order-observation">Observações<textarea id="order-observation" rows="3" placeholder=""></textarea></label><button id="prepare-order">Preparar pedido</button><div id="order-preview" hidden><label for="order-text">Mensagem do pedido:</label><textarea id="order-text" rows="8" readonly></textarea><button id="copy-order">Copiar mensagem</button><a id="whatsapp" target="_blank" rel="noopener noreferrer" hidden>Abrir no WhatsApp ↗</a><p id="phone-note"></p></div></div>';document.body.append(bagDialog);
...
let updateBag=function(){try{localStorage.setItem("mimo-kids-bag",JSON.stringify(bag))}catch{}
  ...
  bag.forEach((x,index)=>{const row=document.createElement('div');row.className='bag-item';
    ...
    const info=document.createElement('div');info.className='bag-item-info';const name=document.createElement('strong');name.textContent=x.name;const meta=document.createElement('span');meta.textContent=`cod. ${items[x.id].code.toLowerCase()} · Tamanho: ${x.size}`;
  ...
  const{discount,total:bagTotal}=calcBagTotal();
  if(discount>0){const discountRow=document.createElement('p');discountRow.className='bag-discount';discountRow.textContent='Promoção linha Jack (3 conjuntos por R$ 125 reais): -'+discount.toLocaleString('pt-BR',{style:'currency',currency:'BRL'});container.append(discountRow)}
  const summary=document.createElement('div');summary.className='bag-total';const summaryLabel=document.createElement('span');summaryLabel.textContent='Total:';const summaryValue=document.createElement('strong');summaryValue.textContent=bagTotal.toLocaleString('pt-BR',{style:'currency',currency:'BRL'});summary.append(summaryLabel,summaryValue);container.append(summary);
