...
const bagDialog=document.createElement('dialog');bagDialog.className='bag-dialog';bagDialog.setAttribute('aria-labelledby','bag-title');bagDialog.innerHTML='<button class="bag-close close" aria-label="Fechar carrinho">×</button><div class="bag-content"><div class="bag-header-row"><div class="bag-header-text"><p class="eyebrow">SEUS MIMOS</p><h2 id="bag-title">Meu carrinho</h2></div><a class="bag-brand brand-logo" href="#" aria-label="Mimo Kids Presentes, início"><svg class="brand-mark" viewBox="144 524 592 628" aria-hidden="true"><image href="assets/logo-mimo-kids.png" width="900" height="1600"/></svg><span class="brand-wordmark"><strong>Mimo Kids</strong><small>Presentes</small></span></a></div><p class="demo-note">Escolha seus mimos e fale com a gente para confirmar tamanhos, valores e disponibilidade.</p><div id="bag-items"></div><p id="bag-status" role="status"></p><label class="order-observation" for="order-observation">Observações<textarea id="order-observation" rows="3" placeholder=""></textarea></label><button id="prepare-order">Preparar pedido</button><div id="order-preview" hidden><label for="order-text">Mensagem do pedido:</label><textarea id="order-text" rows="8" readonly></textarea><button id="copy-order">Copiar mensagem</button><a id="whatsapp" target="_blank" rel="noopener noreferrer" hidden>Abrir no WhatsApp ↗</a><p id="phone-note"></p></div></div>';document.body.append(bagDialog);
